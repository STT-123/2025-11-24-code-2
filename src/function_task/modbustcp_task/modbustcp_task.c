...
/*===========TEST=======================*/
void *ModbusTCPServerTESTTask(void *arg)
{
    int rc;
    unsigned int i;
    fd_set refset;  //所有监听的socket）
    ...
    int server_socket;  // 服务器监听socket
    int master_socket;  // 客户端连接socket
    unsigned char modbus_ip_test[16] = "192.168.4.100";

    // 使用新的组合函数，既设置临时IP又更新永久配置
    int ret = set_ip_address("eth2", modbus_ip_test);
    if (ret != 0) {
        LOG("[ModbusTcp] IP设置失败,但继续尝试创建modbus上下文\n");
    } else {
        LOG("[ModbusTcp] IP设置成功\n");
...
void modbusTcpServerTaskCreate(void)
{
    ...
    do
    {
        ret = pthread_create(&NetConfig_TASKHandle_TEST, NULL, ModbusTCPServerTESTTask, NULL);
        if (ret != 0){
            LOG("[ModbusTcp] Failed to create NETConfigTask thread : %s", strerror(ret));
            sleep(1);
        }else{
